...
class MRQAP():
    ...
    def init(self):
        ...
        for k,x in self.X.items():
            if k == list(self.Y.keys())[0]:
                printf('ERROR: Idependent variable cannot be named \'[}\''.format(list(self.Y.keys())[0]), self.logfile)
                sys.exit(0)
    ...
    def return_betas(self):
        ...
        for k,v in self.betas.items():
            ...
            t_statistic, p_value = stats.ttest_1samp(v, 0)
            if p_value< 0.05 and k!='Intercept':
                betas_agg[k] = (
                    sum(v)/len(v),round(np.std(v),6),True
                )
        # dict: mean, std
        mean_beta, std_beta= (sum(v[0] for v in betas_agg.values()),
                              np.sqrt(np.sum(np.square([v[1] for v in betas_agg.values()]))))
    ...
    def _summary_ols(self):
        ...
        '''
        printf('', self.logfile)
        printf('=== Summary OLS (original) ===\n{}'.format(self.model.summary()), self.logfile)
        printf('', self.logfile)
        printf('# of Permutations: {}'.format(self.npermutations), self.logfile)
    ...
    def _summary_betas(self):
        ...
        '''
        printf('', self.logfile)
        printf('=== Summary beta coefficients ===', self.logfile)
        printf('{:20s}{:>10s}{:>10s}{:>10s}{:>10s}{:>12s}{:>12s}{:>12s}{:>12s}{:>12s}'.format('INDEPENDENT VAR.','MIN','MEDIAN','MEAN','MAX','STD. DEV.','B.COEFF.','As Large', 'As Small', 'P-VALUE'), self.logfile)
        for k,v in self.betas.items():
            ...
            printf('{:20s}{:10f}{:10f}{:10f}{:10f}{:12f}{:12f}{:12f}{:12f}{:12f}'.format(k,min(v),sorted(v)[int(len(v)/2)],sum(v)/len(v),max(v),round(np.std(v),6),beta,aslarge,assmall,round(float(pstats),2)), self.logfile)
    ...
    def _summary_tvalues(self):
        ...
        '''
        printf('', self.logfile)
        printf('=== Summary T-Values ===', self.logfile)
        printf('{:20s}{:>10s}{:>10s}{:>10s}{:>10s}{:>12s}{:>12s}{:>12s}{:>12s}'.format('INDEPENDENT VAR.','MIN','MEDIAN','MEAN','MAX','STD. DEV.','T-TEST','As Large', 'As Small'), self.logfile)
        for k,v in self.tvalues.items():
            ...
            printf('{:20s}{:10f}{:10f}{:10f}{:10f}{:12f}{:12f}{:12f}{:12f}'.format(k,min(v),sorted(v)[int(len(v)/2)],sum(v)/len(v),max(v),round(np.std(v),6),round(float(tstats),2),aslarge,assmall), self.logfile)

    def _ttest(self):
        printf('')
        printf('========== T-TEST ==========')
        printf('{:25s} {:25s} {:25s} {:25s}'.format('IND. VAR.','COEF.','T-STAT','P-VALUE'))
    ...
    def plot(self,coef='betas',fn=None):
        ...
        ranges = list(range(ncols, m, ncols))
...
if __name__ == "__main__":
    ...
    B = np.ones((4, 4))
    model = MRQAP({"test":B},{"test_i":A},npermutations=1000)
    model.mrqap()
    model.summary()
