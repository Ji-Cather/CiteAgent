...
class ArticleManager(BaseManager):
    ...
    different_tag_rate: float = 0.3 # the rate of different tag author in the bfs search
    ...
    def load_data(cls,
        ...
        if "settle_country_random" in experiment:
            for author, info in author_data.items():
                info["country"] = random.choice(countrys_available)
...
"""
        ...
        try:
            ...
            for author_info in authors:
                ...
                if "settle_country_random" in self.experiment:
                    self.author_data[str(index)]["country"] = random.choice(countrys_available)
...
"""         
    ...
    def get_llm_author(self,
        ...
        if len(authors) < threshold:
            ...
            for tag in tags:
                ...
                for same_tag_author in same_tag_author_ids:
                    ...
                    add_diff = False    
                    while(self.different_tag_author and random.random() < self.different_tag_rate \
                           and not add_diff and diff_author_id_p < len(different_tag_author_ids)):
        ...
        template ="""
Here's some of the authors for topic: {topic}
