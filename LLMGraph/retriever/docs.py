...
class GraphVectorRetriever(VectorStoreRetriever):
    ...
    def _get_relevant_documents(
        ...
        search_kwargs = {
            "k": self.search_kwargs.get("recall_k", self.search_kwargs.get("k",20)
                                        ),
        }
        if "similarity" in self.search_type:
            ...
            if len(docs_and_similarities)> self.search_kwargs.get("k"):
                docs_and_similarities = random.sample(docs_and_similarities, self.search_kwargs.get("k"))
            if score_cite:
